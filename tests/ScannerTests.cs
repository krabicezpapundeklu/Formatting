namespace Krabicezpapundeklu.Formatting.Tests
{
    ...
    class ScannerTests
    {
        ...
        [Row(@"\}", new[] { "<Text, 0, 2: }>", "<EndOfInput, 2, 2>" })]
        [Row(@"\\", new[] { @"<Text, 0, 2: \>", "<EndOfInput, 2, 2>" })]
        [Row("xxx{yyy}zzz", new[] {"<Text, 0, 3: xxx>", "<{, 3, 4: {>", "<Text, 4, 7: yyy>", "<}, 7, 8: }>", "<Text, 8, 11: zzz>", "<EndOfInput, 11, 11>" })]
        ...
        {
            Assert.AreElementsEqual(expectedTokens,
                Helpers.Tokenize(new Scanner(input) {State = ScannerState.ScanningText}));
        }
        ...
        {
            Assert.AreElementsEqual(expectedTokens,
                Helpers.Tokenize(new Scanner(input) { State = ScannerState.ScanningTokens }));
        }
    }
}
